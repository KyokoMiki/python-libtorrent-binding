...
set_module_urls() {
	...
	#
	boost_version="$(curl -s "https://api.github.com/repos/boostorg/boost/tags" | jello -r '_[0].name' | grep -Eom1 '1.(.*).0$')"
	...
	# curl -s "https://api.github.com/repos/ratanakvlun/deluge-ltconfig/releases" | jello -r '_.assets[0].name' | grep -Eom1 '1.(.*).0$'
	ltconfig_version="$(curl -s "https://api.github.com/repos/ratanakvlun/deluge-ltconfig/releases/latest" | jello -r '_.assets[0].name' | grep -Eom1 'ltConfig-(.*).egg' | sed 's/ltConfig-//;s/.egg//')"
